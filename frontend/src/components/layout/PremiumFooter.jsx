...
export default function PremiumFooter() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await apiGet('/api/settings/branding')
        if (res?.headerLogo) {
          const url = res.headerLogo.startsWith('http')
            ? res.headerLogo
            : `${API_BASE || ''}${res.headerLogo}`
          setLogoUrl(url + '?v=2')
        }
      } catch {}
    })()
    ...
    <footer className="sf-root">
        ...
        <div className="sf-trust-inner">
          {[
            ...
            <div key={f.label} className="sf-trust-item">
              ...
              <span className="sf-trust-label">{f.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="sf-main">
        ...
        <div className="sf-brand-col">
          <Link to="/">
            <img src={logoUrl || '/BuySial2.png?v=2'} alt="BuySial" className="sf-brand-logo" />
          </Link>
          ...
          <div className="sf-social-row">
            {SOCIAL_ICONS.map(({ key, icon, label }) => {
              ...
              return href
                ? <a key={key} href={href} target="_blank" rel="noopener noreferrer" className="sf-social-btn" title={label}>{icon}</a>
                : <span key={key} className="sf-social-btn sf-social-dim" title={label}>{icon}</span>
            })}
          </div>
        </div>
        ...
        <div className="sf-cols">
          {SECTIONS.map((sec, idx) => (
            <div key={idx} className="sf-col">
              <h4 className="sf-col-title">{sec.title}</h4>
              {sec.links.map((link, i) => (
                <Link key={i} to={link.to} className="sf-col-link">{link.label}</Link>
              ))}
            </div>
          ))}
        </div>
        ...
        <div className="sf-accordion">
          {SECTIONS.map((sec, idx) => {
            ...
            return (
              <div key={idx} className="sf-acc-item">
                <button className="sf-acc-btn" onClick={() => toggle(idx)}>
                  <span>{sec.title}</span>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', flexShrink:0}}>
                    <polyline points="6 9 12 15 18 9"/>
                  </svg>
                </button>
                {open && (
                  <div className="sf-acc-links">
                    {sec.links.map((link, i) => (
                      <Link key={i} to={link.to} className="sf-col-link">{link.label}</Link>
                    ))}
                  </div>
      ...
      <div className="sf-payments">
        ...
        <div className="sf-payment-badges">
          <div className="sf-pay"><span style={{fontSize:12,fontWeight:700,color:'#253B80'}}>Pay</span><span style={{fontSize:12,fontWeight:700,color:'#179BD7'}}>Pal</span></div>
          <div className="sf-pay">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 750 471" style={{width:36,height:22}}><path d="M278.198 334.228l33.36-195.763h53.358L331.57 334.228h-53.372zM524.307 142.687c-10.57-3.966-27.135-8.222-47.822-8.222-52.726 0-89.863 26.551-90.181 64.604-.318 28.129 26.514 43.822 46.754 53.185 20.771 9.598 27.752 15.716 27.652 24.283-.133 13.123-16.586 19.115-31.924 19.115-21.355 0-32.701-2.967-50.225-10.273l-6.878-3.111-7.487 43.822c12.463 5.467 35.508 10.199 59.438 10.445 56.09 0 92.502-26.248 92.916-66.885.199-22.27-14.016-39.215-44.801-53.188-18.65-9.056-30.072-15.099-29.951-24.269 0-8.137 9.668-16.838 30.56-16.838 17.446-.271 30.088 3.534 39.936 7.5l4.781 2.259 7.232-42.627M661.615 138.464h-41.23c-12.773 0-22.332 3.486-27.941 16.234l-79.244 179.402h56.031s9.159-24.121 11.231-29.418c6.123 0 60.555.084 68.336.084 1.596 6.854 6.492 29.334 6.492 29.334h49.512l-43.187-195.636zM232.903 138.464L180.664 271.96l-5.565-27.129c-9.726-31.274-40.025-65.157-73.898-82.12l47.767 171.204 56.455-.066 84.004-195.386-56.524.001" fill="#1A1F71"/><path d="M131.92 138.464H45.879l-.682 4.073c66.939 16.204 111.232 55.363 129.618 102.415l-18.709-89.96c-3.229-12.396-12.597-16.095-24.186-16.528" fill="#F9A533"/></svg>
          </div>
          <div className="sf-pay">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 30" style={{width:32,height:20}}><circle cx="16" cy="15" r="14" fill="#EB001B"/><circle cx="32" cy="15" r="14" fill="#F79E1B"/><path d="M24 4.27a13.93 13.93 0 0 0-5.2 10.73A13.93 13.93 0 0 0 24 25.73 13.93 13.93 0 0 0 29.2 15 13.93 13.93 0 0 0 24 4.27z" fill="#FF5F00"/></svg>
          </div>
          <div className="sf-pay sf-pay-dark">
            <svg width="13" height="13" viewBox="0 0 814 1000" fill="#fff"><path d="M788.1 340.9c-5.8 4.5-108.2 62.2-108.2 190.5 0 148.4 130.3 200.9 134.2 202.2-.6 3.2-20.7 71.9-68.7 141.9-42.8 61.6-87.5 123.1-155.5 123.1s-85.5-39.5-164-39.5c-76.5 0-103.7 40.8-165.9 40.8s-105.6-57.8-155.5-127.4C58.3 792.6 10.7 664.6 10.7 543.4c0-194.4 126.4-297.5 250.8-297.5 66.1 0 121.2 43.4 162.7 43.4 39.5 0 101.1-46 176.3-46 28.5 0 130.9 2.6 198.3 99.2zm-234-181.5c31.1-36.9 53.1-88.1 53.1-139.3 0-7.1-.6-14.3-1.9-20.1-50.6 1.9-110.8 33.7-147.1 75.8-28.5 32.4-55.1 83.6-55.1 135.5 0 7.8.6 15.7 1.3 18.2 2.6.6 6.4 1.3 10.2 1.3 45.4 0 103.5-30.4 139.5-71.4z"/></svg>
          ...
          <div className="sf-pay">
            ...
            <span style={{fontSize:11,fontWeight:600,color:'#3C4043'}}>Pay</span>
          </div>
        </div>
      </div>
      ...
      <div className="sf-bottom">
        <span>&copy; {currentYear} BuySial. All rights reserved.</span>
        <div className="sf-bottom-links">
          <Link to="/" className="sf-bottom-link">Home</Link>
          <Link to="/privacy" className="sf-bottom-link">Privacy</Link>
          <Link to="/terms" className="sf-bottom-link">Terms</Link>
          <Link to="/contact" className="sf-bottom-link">Contact</Link>
        </div>
      </div>
