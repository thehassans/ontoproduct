      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            <span className="chip" style={{ fontSize: 11, fontWeight: 800 }}>{currency}</span>
            {pendingRemit ? (
              <span className="badge" style={{ fontSize: 11, fontWeight: 800 }}>
                {pendingRemit.status === 'manager_accepted' ? 'Owner Pending' : 'Action Needed'}
              </span>
            ) : null}
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 10 }}>
          <div style={{ border: '1px solid rgba(148, 163, 184, 0.18)', borderRadius: 14, padding: 10, background: 'rgba(245, 158, 11, 0.07)' }}>
            <div className="helper">Assigned (Open)</div>
            <div style={{ fontWeight: 900, fontSize: 18, color: '#f59e0b' }}>{num(d.assigned || 0)}</div>
          </div>
          ...
          <div style={{ border: '1px solid rgba(148, 163, 184, 0.18)', borderRadius: 14, padding: 10, background: 'rgba(34, 197, 94, 0.06)' }}>
            ...
            <div style={{ fontWeight: 900, fontSize: 15, color: '#10b981' }}>{currency} {num(d.collected || 0)}</div>
          </div>
          <div style={{ border: '1px solid rgba(148, 163, 184, 0.18)', borderRadius: 14, padding: 10, background: 'rgba(239, 68, 68, 0.06)' }}>
            <div className="helper">Pending</div>
            <div style={{ fontWeight: 900, fontSize: 15, color: '#ef4444' }}>{currency} {num(d.pendingToCompany || 0)}</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {pendingRemit ? (
              pendingRemit.status === 'manager_accepted' ? (
                <span
                  className="badge"
                  style={{ background: 'rgba(16, 185, 129, 0.15)', borderColor: 'rgba(16, 185, 129, 0.35)', fontWeight: 800 }}
                ...
                <button
                  ...
                  onClick={() => openAcceptModal(pendingRemit._id, name, pendingRemit.amount)}
                  disabled={accepting === pendingRemit._id}
                  style={{ padding: '8px 12px', fontSize: 12, borderRadius: 12, whiteSpace: 'nowrap' }}
                >
            ...
            {pendingRemit?.pdfPath ? (
              <a
                ...
                className="btn"
                style={{ padding: '8px 12px', fontSize: 12, borderRadius: 12, background: '#dc2626', color: 'white', whiteSpace: 'nowrap' }}
          ...
          <button
            className="btn"
            onClick={() => loadHistory(d.id, name)}
            disabled={loadingHistory}
            style={{ padding: '8px 12px', fontSize: 12, borderRadius: 12, background: 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)', color: 'white', whiteSpace: 'nowrap' }}
    ...
    <div className="section" style={{ display: 'grid', gap: 12 }}>
      <div className="page-header" style={isMobileView ? { flexDirection: 'column', alignItems: 'stretch', gap: 10 } : undefined}>
        ...
        <button
          ...
          style={isMobileView ? { width: '100%', borderRadius: 14 } : undefined}
        >
          Pay to Company
        </button>
      </div>
        ...
        <div className="card">
          ...
          <div style={{ display: isMobileView ? 'grid' : 'none', gap: 12 }}>
            {loading ? (
              ...
              drivers.map((d) => <DriverCard key={String(d.id)} d={d} />)
            )}
          </div>
