...
export default function ManagerDashboard(){
  ...
  if (loading) {
    return (
      <div className="section" style={{ maxWidth: 1240, margin: '0 auto', padding: isMobile ? 16 : '24px 20px' }}>
        <div className="card" style={{ padding: 18, borderRadius: 16 }}>
          <div className="section" style={{ opacity: 0.7 }}>Loading dashboard…</div>
        </div>
      </div>
    )
  }
    ...
    <div className="section" style={{ maxWidth: 1240, margin: '0 auto', padding: isMobile ? 16 : '24px 20px' }}>
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">Assigned orders overview</div>
        </div>
        {canCreateDrivers ? (
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            <NavLink className="btn" to="/manager/drivers/create">Create Driver</NavLink>
          </div>
        ) : null}
      </div>

      <div className="grid" style={{gridTemplateColumns: '1fr', gap: 12, alignItems:'start'}}>

      <div className="card" style={{padding:18, borderRadius:16}}>
        {(function(){
          const st = statusExact || {}
          const openOrders = Number(st.pending||0) + Number(st.assigned||0) + Number(st.picked_up||0) + Number(st.in_transit||0) + Number(st.out_for_delivery||0) + Number(st.no_response||0)
          const closedOrders = Number(st.delivered||0) + Number(st.returned||0) + Number(st.cancelled||0)
          const countryCount = Array.isArray(assignedList) ? assignedList.length : 0
          return (
            <div style={{ display:'grid', gap:12 }}>
              <div style={{ display:'flex', justifyContent:'space-between', gap:12, alignItems:'center', flexWrap:'wrap' }}>
                <div style={{ fontWeight:900, fontSize:16 }}>Quick Overview</div>
                <div className="helper" style={{ opacity: 0.75 }}>{countryCount ? `${countryCount} countries` : 'All countries'}</div>
              </div>
              <div className="grid" style={{ gridTemplateColumns:`repeat(auto-fit, minmax(${minTile}px, 1fr))`, gap: tileGap }}>
                <NavLink to="/manager/orders" style={{ textDecoration:'none', color:'inherit' }}>
                  <div style={{ border:'1px solid var(--border)', background:'var(--panel)', borderRadius:16, padding:16, display:'grid', gap:6, minHeight:104 }}>
                    <div className="helper">Total Orders</div>
                    <div style={{ fontSize:valueFontSize, fontWeight:900, color: COLORS.primaryLight }}>{fmtNum(st.total||0)}</div>
                  </div>
                </NavLink>
                <NavLink to="/manager/orders?ship=open" style={{ textDecoration:'none', color:'inherit' }}>
                  <div style={{ border:'1px solid var(--border)', background:'var(--panel)', borderRadius:16, padding:16, display:'grid', gap:6, minHeight:104 }}>
                    <div className="helper">Open Orders</div>
                    <div style={{ fontSize:valueFontSize, fontWeight:900, color: COLORS.warning }}>{fmtNum(openOrders)}</div>
                  </div>
                </NavLink>
                <NavLink to="/manager/orders?ship=delivered" style={{ textDecoration:'none', color:'inherit' }}>
                  <div style={{ border:'1px solid var(--border)', background:'var(--panel)', borderRadius:16, padding:16, display:'grid', gap:6, minHeight:104 }}>
                    <div className="helper">Delivered</div>
                    <div style={{ fontSize:valueFontSize, fontWeight:900, color: COLORS.successDeep }}>{fmtNum(st.delivered||0)}</div>
                  </div>
                </NavLink>
                <NavLink to="/manager/orders?ship=returned" style={{ textDecoration:'none', color:'inherit' }}>
                  <div style={{ border:'1px solid var(--border)', background:'var(--panel)', borderRadius:16, padding:16, display:'grid', gap:6, minHeight:104 }}>
                    <div className="helper">Closed (Delivered/Returned/Cancelled)</div>
                    <div style={{ fontSize:valueFontSize, fontWeight:900, color: COLORS.neutral }}>{fmtNum(closedOrders)}</div>
                  </div>
                </NavLink>
              </div>
            </div>
      ...
      <div className="card" style={{padding:18, marginBottom:12, borderRadius:16}}>
        {(function(){
          ...
          function Tile({ title, valueEl, chipsEl, color }){
            return (
              <div className="tile" style={{display:'grid', gap:6, padding:16, textAlign:'left', border:'1px solid var(--border)', background:'var(--panel)', borderRadius:16, minHeight:104}}>
      ...
      <details style={{ border:'1px solid var(--border)', borderRadius:16, background:'var(--panel)', overflow:'hidden' }}>
        <summary style={{ cursor:'pointer', padding:'14px 16px', fontWeight:900, listStyle:'none' }}>
          Driver Report
        </summary>
        {/* Driver Report by Country (Access Countries) */}
        <div className="card" style={{padding:18, marginBottom:0, borderRadius:0, border:'none', boxShadow:'none'}}>
        ...
        <div className="section" style={{display:'grid', gap:12}}>
          {(COUNTRY_LIST||[]).map(c=>{
            ...
            const tiles = [
              ...
              { key:'deliv_co', title:'Delivered to Company', val: Number(d.deliveredToCompany||0), isAmount: true, to:`/manager/transactions/drivers` },
              { key:'pending_co', title:'Pending Delivery to Company', val: Number(d.pendingToCompany||0), isAmount: true, to:`/manager/transactions/drivers` },
            ]
            const visibleTiles = tiles.filter(t=> Number(t.val||0) > 0)
            return (
              <div key={c} className="panel" style={{border:'1px solid var(--border)', borderRadius:16, padding:12, background:'var(--panel)'}}>
                ...
                {visibleTiles.length === 0 ? (
                  ...
                  <div className="grid" style={{gridTemplateColumns:`repeat(auto-fit, minmax(${minTile}px, 1fr))`, gap: tileGap}}>
                    {visibleTiles.map(t => {
                      ...
                      return (
                        <NavLink key={t.key} className="tile" to={t.to} style={{display:'grid', gap:6, padding:16, textAlign:'left', border:'1px solid var(--border)', background:'var(--panel)', borderRadius:16, minHeight:104, textDecoration:'none', color:'inherit', cursor:'pointer'}}>
      ...
      <details style={{ border:'1px solid var(--border)', borderRadius:16, background:'var(--panel)', overflow:'hidden' }}>
        ...
        <div className="card" style={{padding:18, marginTop:0, borderRadius:0, border:'none', boxShadow:'none'}}>
        {(function(){
        ...
        <div className="section" style={{overflowX:'auto'}}>
          <div style={{display:'flex', gap:12, minWidth:700}}>
            {assignedList.map(ctry=>{
              ...
              return (
                <div key={ctry} className="mini-card" style={{border:'1px solid var(--border)', borderRadius:16, padding:'12px 14px', background:'var(--panel)', minWidth:280}}>
                  ...
                  <div style={{display:'grid', gap:6}}>
                    ...
                    <div style={{display:'flex', alignItems:'center', justifyContent:'space-between'}}>
                      <div className="helper">Delivered to Company</div>
                      <a className="link" href={`/manager/transactions/drivers`}>{currency} {Math.round(m.deliveredToCompany||0).toLocaleString()}</a>
                    </div>
                    <div style={{display:'flex', alignItems:'center', justifyContent:'space-between'}}>
                      <div className="helper">Pending to Company</div>
                      <a className="link" href={`/manager/transactions/drivers`}>{currency} {Math.round(m.pendingToCompany||0).toLocaleString()}</a>
                    </div>
                  </div>
                </div>
              )
