...
export default function Drivers(){
  ...
  const [form, setForm] = useState({ firstName:'', lastName:'', email:'', password:'', phone:'', country: DEFAULT_COUNTRY.name, city:'', assignedManager:'' })
  ...
  const [editModal, setEditModal] = useState({ open:false, busy:false, error:'', driver:null, firstName:'', lastName:'', email:'', phone:'', country: DEFAULT_COUNTRY.name, city:'', password:'', assignedManager:'' })
  ...
  const managerLabelById = useMemo(()=>{
    ...
    for (const u of managers){
      ...
      if (!id) continue
      const name = `${u?.firstName||''} ${u?.lastName||''}`.trim() || u?.email || 'Manager'
      m.set(id, name)
    }
    return m
  }, [managers])
  ...
  function openEdit(driver){
    ...
    setEditModal({
      ...
      assignedManager: String(d.assignedManager || ''),
    })
  }
  ...
  async function saveEdit(){
    ...
    try{
      ...
      await apiPatch(`/api/users/drivers/${uid}`, {
        ...
        assignedManager: editModal.assignedManager || null,
      })
      setEditModal({ open:false, busy:false, error:'', driver:null, firstName:'', lastName:'', email:'', phone:'', country: DEFAULT_COUNTRY.name, city:'', password:'', assignedManager:'' })
      await loadDrivers(q)
    }catch(e){
  ...
  async function loadDrivers(query=''){
    ...
    try{
      ...
      setRows((data.users||[]).map(u => ({
        ...
        assignedManager: String(u?.assignedManager || ''),
      })))
    }catch(_e){ setRows([]) }
  ...
  useEffect(()=>{
    ...
    ;(async ()=>{
      try{
        const r = await apiGet('/api/users/managers')
        if (!alive) return
  ...
  async function onSubmit(e){
    ...
    try{
      ...
      if (!isAllowedCountry) {
        setLoading(false)
        setPhoneError('Phone must start with +971 (UAE), +968 (Oman), +966 (KSA), +973 (Bahrain), +965 (Kuwait), +974 (Qatar), +91 (India), +92 (Pakistan), +962 (Jordan), +1 (USA/Canada), +44 (UK) or +61 (Australia)')
        setMsg('')
        return
      }
      ...
      setForm({ firstName:'', lastName:'', email:'', password:'', phone:'', country: DEFAULT_COUNTRY.name, city:'', assignedManager:'' })
    ...
    <div className="section">
      ...
      <div className="card">
        ...
        <form onSubmit={onSubmit} className="section" style={{display:'grid', gap:12}}>
            ...
            <div>
              ...
              <div className={`PhoneInput ${phoneError? 'input-error':''}`}>
                <PhoneInput
                  ...
                  countries={['AE','OM','SA','BH','IN','KW','QA','PK','JO','US','GB','CA','AU']}
              ...
              <div className={`helper-text ${phoneError? 'error':''}`}>{phoneError || 'Only UAE, Oman, KSA, Bahrain, Pakistan, Kuwait, Qatar, India, Jordan, USA, UK, Canada or Australia numbers allowed'}</div>
            </div>
            <div>
          ...
          <div className="form-grid">
            <div>
              <div className="label">Assign To Manager (optional)</div>
              <select className="input" name="assignedManager" value={form.assignedManager} onChange={onChange}>
                <option value="">-- Unassigned --</option>
                {managers.map(m => (
                  <option key={String(m._id||m.id)} value={String(m._id||m.id)}>{`${m.firstName||''} ${m.lastName||''}`.trim() || m.email}</option>
                ))}
              </select>
            </div>
            <div>
          ...
          <table style={{width:'100%', borderCollapse:'separate', borderSpacing:0}}>
            <thead>
              <tr>
                ...
                <th style={{textAlign:'left', padding:'10px 12px'}}>Manager</th>
              ...
              {loadingList ? (
                ...
                rows.map(u=> (
                  <tr key={u.id} style={{borderTop:'1px solid var(--border)'}}>
                    ...
                    <td style={{padding:'10px 12px'}}>{u.assignedManager ? (managerLabelById.get(String(u.assignedManager)) || 'Manager') : '-'}</td>
      ...
      >
        <div style={{display:'grid', gap:12}}>
          ...
          <div>
            <div className="label">Assign To Manager (optional)</div>
            <select className="input" value={editModal.assignedManager} onChange={e=> setEditModal(m=>({ ...m, assignedManager: e.target.value }))}>
              <option value="">-- Unassigned --</option>
              {managers.map(m => (
                <option key={String(m._id||m.id)} value={String(m._id||m.id)}>{`${m.firstName||''} ${m.lastName||''}`.trim() || m.email}</option>
              ))}
