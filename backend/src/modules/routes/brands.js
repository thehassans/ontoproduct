...
router.delete('/:id', auth, allowRoles('admin', 'user', 'manager', 'web_designer'), async (req, res) => {
  try {
    if (!(await checkManagerBrandPerm(req))) {
      return res.status(403).json({ message: 'Not authorized to manage brands' })
    }
...
router.post('/:id/logo', auth, allowRoles('admin', 'user', 'manager', 'web_designer'), brandUpload.single('logo'), async (req, res) => {
  try {
    if (!(await checkManagerBrandPerm(req))) {
      return res.status(403).json({ message: 'Not authorized to manage brands' })
    }
