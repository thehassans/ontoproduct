...
router.post(
  ...
  async (req, res) => {
    try {
      const { firstName, lastName, email, password, phone, designerPermissions } = req.body || {};
      if (!firstName || !lastName || !email || !password)
        return res.status(400).json({ message: "Missing required fields" });
      const exists = await User.findOne({ email });
      if (exists) return res.status(400).json({ message: "Email already in use" });
      const perms = typeof designerPermissions === "object" && designerPermissions ? designerPermissions : {};
      ...
      try {
        const io = getIO();
        io.to(`workspace:${req.user.id}`).emit("designer.created", { id: String(designer._id) });
      } catch {}
      res.status(201).json({
        message: "Designer created",
        user: { id: designer._id, firstName, lastName, email, role: "designer", designerPermissions: designer.designerPermissions },
      });
    } catch (err) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const users = await User.find(filter, "firstName lastName email phone designerPermissions createdAt")
        .sort({ createdAt: -1 })
        .lean();
      res.json({ users: users.map((u) => ({ ...u, id: u._id })) });
    } catch (err) {
      res.status(500).json({ message: "Failed to fetch designers", error: err?.message });
    }
  }
);

// Update designer (admin, user)
router.patch(
  "/designers/:id",
  auth,
  allowRoles("admin", "user"),
  async (req, res) => {
    try {
      const designer = await User.findOne({ _id: req.params.id, role: "designer" });
      if (!designer) return res.status(404).json({ message: "Designer not found" });
      if (req.user.role !== "admin" && String(designer.createdBy) !== String(req.user.id))
        return res.status(403).json({ message: "Not allowed" });
      const { firstName, lastName, email, phone, password, designerPermissions } = req.body || {};
      if (firstName) designer.firstName = firstName.trim();
      if (lastName) designer.lastName = lastName.trim();
      if (email) designer.email = email.trim().toLowerCase();
      if (typeof phone === "string") designer.phone = phone.trim();
      if (password && password.trim()) designer.password = password.trim();
      if (designerPermissions && typeof designerPermissions === "object") {
        designer.designerPermissions = {
          canManageHomeHeadline: !!(designerPermissions.canManageHomeHeadline ?? designer.designerPermissions?.canManageHomeHeadline),
          canManageProductHeadline: !!(designerPermissions.canManageProductHeadline ?? designer.designerPermissions?.canManageProductHeadline),
          canManageHomeBanners: !!(designerPermissions.canManageHomeBanners ?? designer.designerPermissions?.canManageHomeBanners),
          canManageHomeMiniBanners: !!(designerPermissions.canManageHomeMiniBanners ?? designer.designerPermissions?.canManageHomeMiniBanners),
          canManageBrands: !!(designerPermissions.canManageBrands ?? designer.designerPermissions?.canManageBrands),
          canManageExploreMore: !!(designerPermissions.canManageExploreMore ?? designer.designerPermissions?.canManageExploreMore),
        };
      }
      await designer.save();
      try { const io = getIO(); io.to(`workspace:${req.user.id}`).emit("designer.updated", { id: String(designer._id) }); } catch {}
      res.json({ message: "Designer updated", user: { id: designer._id, firstName: designer.firstName, lastName: designer.lastName, email: designer.email, phone: designer.phone, designerPermissions: designer.designerPermissions } });
    } catch (err) {
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      try { const io = getIO(); io.to(`workspace:${req.user.id}`).emit("designer.deleted", { id: req.params.id }); } catch {}
